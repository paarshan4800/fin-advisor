...
db = client["transaction_db_flatten"]

users_collection = db["users"]          # keep as-is
transactions_collection = db["transactions"]  # single merged collection

numberOfRecords = 30000
num_users = max(30, numberOfRecords // 1000)      # ~1 user per 1000 txns (min 30)
num_merchants = 750
...
def create_accounts(user_ids):
    # one account per user, generated in-memory
    accounts = []
    for uid in user_ids:
        accounts.append({
            "_id": str(uuid.uuid4()),
            "user_id": uid,
            "account_number": fake.iban(),  # synthetic
        })
    return accounts
...
def create_transactions_merged(user_ids, accounts, merchants):
    ...
    for _ in range(numberOfRecords):
        ...
        # Build single-document transaction with embedded account/merchant
        txn_doc = {
            "_id": str(uuid.uuid4()),
            "transaction_id": str(uuid.uuid4()),
            "user_id": user_id,

            # Embedded account that initiated the txn
            "from_account": {
                "_id": from_acc["_id"],
                "user_id": from_acc["user_id"],
                "user_name": users_by_id.get(from_acc["user_id"], {}).get("name"),  # <-- added
                "account_number": from_acc["account_number"],
            },

            # Embedded counterparty account OR merchant (one of these is None)
            "to_account": to_account_obj,  # None for merchant payments
            "merchant": (
...
if __name__ == "__main__":
    ...
    print(f"Generated {len(user_ids)} users (fresh), {len(accounts)} in-memory accounts, "
          f"{len(merchants)} in-memory merchants, and {total} merged transactions in 'transactions'.")
